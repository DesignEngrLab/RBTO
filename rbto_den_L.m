%%% rbto_den_L(60,60,3,1.5,dismax)
function rbto_den_L(nelx,nely,penal,rmin,dismax,reli,lwE,fn,title)
    ...
    dof = 2*(nelx)*(nely+1)+2*(nely*3/4+1);
    upE = 1;
    ...
    fval(1,1) = 1;
    main_loop = 0;
    while mpptol > 0.001 && main_loop < 20
        mppold = mpp;
        loop = 0;
        ...
        while change > 0.001
            loop = loop + 1;
            [U] = FEL(nelx,nely,xphy,penal,KE,E,dof);
            ...
            x = xnew;
            xphy(passive == 1) = 0.001;
            
            % Results
            disp([' It.: ' sprintf('%4i',loop) ' Obj.: ' sprintf('%6.4f',c) ...
                  ' Vol Frac.: ' sprintf('%6.4f',sum(sum(xphy))/(3/4*nelx*nely)) ...
...
%             pause(1e-6);
    ...
    fprintf(fileID,'%s',title);
    fprintf(fileID,'\nVol Frac.: %6.4f',sum(sum(xphy))/(3/4*nelx*nely));
    fprintf(fileID,'\nmain_loop: %4i',main_loop);
    fclose(fileID);
    %%%
    disp('END OPTIMIZATION');
    rbto_mc_L(nelx, nely, penal, xphy, dismax, dof, upE, lwE);
...
function mpp = find_mpp(colPoints, eigV, eigF, nKL, nelx, nely, x, penal,...
    ...
    res = CODES.reliability.iform(g,2,reli,'solver','hmv','Tinv',Tinv);
    disp(res)
    mpp = res.MPTP;
